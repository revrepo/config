function BOOMR_check_doc_domain(e) {
    var t;
    if (!e) {
        if (window.parent === window ||!document.getElementById("rev-rum-is")) {
            return true
        }
        e = document.domain
    }
...
function BOOMR_in_iframe() {
    try {
        return (window.self !== window.top && !document.getElementById("rev-rum-is"))
    } catch (e) {
        return true
    }
}
...
(function(e) {
    if (BOOMR_in_iframe())
        return;
    var t, n, r, i;
    if (e.parent !== e && document.getElementById("rev-rum-is") && document.getElementById("rev-rum-is").nodeName.toLowerCase() === "script") {
        e = e.parent;
        i = document.getElementById("rev-rum-is").src
    }
    r = e.document;
...
BOOMR.init({
    ...
    beacon_url: 'http://boom.revsw.net/service'
});
