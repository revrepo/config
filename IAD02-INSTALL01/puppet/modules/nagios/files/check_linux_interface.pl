...
my $virt = 0;
my $virtwhatresults = `/usr/sbin/virt-what`;
if ( $virtwhatresults )
    { 
        $virt = 1;
    }
...
if(defined($duplex)){
    ...
    quit "UNKNOWN", "Duplex not found in '$ethtool' output!" unless ($bond || $virt);
}
...
if(defined($speed)){
    ...
    quit "UNKNOWN", "Speed not found in '$ethtool' output!" unless ( $bond || $virt);
}
