...
#
# UFW configuration script for CDS servers 
#
...
# Allow CDS port 9000/TCP port from monitoring servers
# Allow from SJC02-MONITOR02
ufw allow proto tcp from 54.193.118.76/32 to any port 9000
# Allow from IAD02-MONITOR01
ufw allow proto tcp from 54.88.56.156/32 to any port 9000


# Allow CDS port 9000/TCP port from API servers
# Allow from IAD02-API02
ufw allow proto tcp from 54.88.69.242/32 to any port 9000
# Allow from IAD02-API01
ufw allow proto tcp from 54.165.134.123/32 to any port 9000
# Allow from IAD02-API03
ufw allow proto tcp from 54.165.8.28/32 to any port 9000
# Allow from IAD02-STATSAPI01
ufw allow proto tcp from 52.1.122.69/32 to any port 9000


# Allow CDS port 9000/TCP access from home IPs of Rev admins
# Allow from VICTOR-HOME-IP
ufw allow proto tcp from 73.223.31.210/32 to any port 9000
# Allow from JON-HOME-IP-SONIC
ufw allow proto tcp from 157.131.200.6/32 to any port 9000
# Allow from JON-HOME-IP-COMCAST
ufw allow proto tcp from 98.207.92.71/32 to any port 9000
# Allow from JON2-HOME-IP
ufw allow proto tcp from 72.199.237.160/32 to any port 9000


# Allow CDS API access from the DEV environment for software deployment?? Do we need this?
ufw allow proto tcp from 184.105.140.178/32 to any port 9000

# Allow CDS API access from the SJ office network
ufw allow proto tcp from 156.39.125.3/32 to any port 9000
